...
import pps.covid_sim.model.places.OpenPlaces.{Field, Park}
...
private[hobbies] case class FootballTeamCreation() {
  ...
  def create(city: City,
    ...
    while (numWorker < totalWorker) {
      ...
      val footballTeam: FootballTeam = FootballTeam(city, !closedPlaceInLockdown.contains(classOf[FootballTeam]), fields)
      for (field <- fields) {
        ...
        if (numWorker < totalWorker) {
          workers.slice(numWorker, bound).foreach(worker => { // add WorkPlan to each worker
            val plan: WorkPlan[Field] = WorkPlan(!closedPlaceInLockdown.contains(classOf[FootballTeam]))
