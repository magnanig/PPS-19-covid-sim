...
import pps.covid_sim.model.transports.PublicTransports.{Bus, BusLine}
...
class LineTest {
  ...
  val city: City = new City(1,"Forlì",50,new Province(3,"FC","Forlì",Locality.Region.EMILIA_ROMAGNA));

  val lineBus: BusLine = BusLine(2,2,HoursInterval(15,16))
  lineBus.setCoveredCities(Set(city))
  ...
  def testBusTurnout(): Unit = {
    assertEquals(true,lineBus.reach(place))
