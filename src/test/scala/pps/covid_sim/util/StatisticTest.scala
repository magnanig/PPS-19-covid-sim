...
class StatisticTest {
  ...
  def testMiddleAgeCurrentPositive(): Unit = {
    ...
    (1 to 100).foreach(_ => {
      people = samples.PersonTmp(RandomGeneration.randomBirthDate(0, 100), Cities.CERVIA) :: people
    })
  ...
  def testCurrentPositive(): Unit = {
    ...
    CitiesObject.getCities(Region.FRIULI_VENEZIA_GIULIA)
      .foreach(city => people = samples.PersonTmp(date, city, infected = true) :: people)
  ...
  def testRecoveredPeople(): Unit = {
    ...
    CitiesObject.getCities(Region.EMILIA_ROMAGNA)
      .foreach(city => people = samples.PersonTmp(date, city, recovered = true) :: people)
    CitiesObject.getCities(Region.TRENTINO_ALTO_ADIGE)
      .foreach(city => people = samples.PersonTmp(date, city) :: people)
  ...
  def testConfirmedCases(): Unit = {
    ...
    CitiesObject.getCities(Region.CALABRIA).foreach(city => people = samples.PersonTmp(date, city, infected = true) :: people)
